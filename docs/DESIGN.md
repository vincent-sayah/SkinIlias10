...
- La version `1.1.6` place le logo eForm Marine dans le bandeau haut gauche via `HeaderIcon.svg` et une version PNG horizontale optimisee pour l'en-tete.
...
- Le dossier `images` fournit des icones SVG `icon_*.svg` issues des pictogrammes ILIAS 10 Delos d'origine, recolorisees avec une separation nette 50% bleu marine / 50% rouge pour les objets courants : cours, categorie, groupe, dossier, fichier, test, forum, module, SCORM, exercice, wiki, blog, session, sondage, lien web et pool de questions.
